...
import { itensPedido, setItensPedido } from "./cardapio.js";
...
function criarItemCarrinho(item, orderId) {
  ...
  card.innerHTML = `
    ...
    <button class="lixeira" data-product-id="${item.product_id}">
  ...
  // Modifique a função delete_item no seu frontend (onde está o evento do botão lixeira)
  card.querySelector(".lixeira").addEventListener("click", async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:5000/excluir/${orderId}/${item.product_id}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        // Atualiza o estado local baseado na resposta do servidor
        if (result.quantity === 0) {
          delete itensPedido[item.product_id];
        } else {
          itensPedido[item.product_id] = result.quantity;
        }
      }
...
function popularCarrinho(order) {
  ...
  if (!order || !order.itens || order.itens.length === 0) {
    ...
    botaoCarrinhoImg.src = "../../static/img/interface/cloche-atento.png";
  }

  order.itens.forEach((item) => {
    container.appendChild(criarItemCarrinho(item, order.order_id));
  });
